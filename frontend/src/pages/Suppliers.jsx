...
import { Plus, Pencil, Trash2, Mail, Phone, MapPin, Building } from 'lucide-react';
...
export default function Suppliers() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await supplierService.getAll({ search: debouncedSearch, page, limit: 10 });
      setRows(data.data);
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div>
          ...
          {row.company_name && <p className="text-xs text-gray-400 flex items-center gap-1"><Building size={11} /> {row.company_name}</p>}
        </div>
      ),
    },
    {
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Edit Supplier' : 'Add Supplier'}>
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-3 mt-4">
            ...
            <Button type="submit" disabled={saving}>{saving ? 'Saving...' : editingId ? 'Update Supplier' : 'Add Supplier'}</Button>
          </div>
        </form>
      </Modal>
